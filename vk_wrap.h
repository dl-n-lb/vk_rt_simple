...
#ifndef VKW_CALLOC
#define VKW_CALLOC calloc
#endif

#ifndef VKW_FREE
#define VKW_FREE free
#endif

#ifndef VKW_REALLOC
#define VKW_REALLOC realloc
#endif
...
#define vkw_da_push(p_arr, val) \
  do {									\
	...
	(p_arr)->cap = 1;						\
      ...
      (p_arr)->cap *= 2;						\
      (p_arr)->data = VKW_REALLOC((p_arr)->data,			\
...
vkw_descriptor_pool_create(VkDevice device, uint32_t sets, uint32_t cnt,
			   vkw_pool_size_ratio ratios[restrict cnt]) {
  VkDescriptorPoolSize *sizes = VKW_CALLOC(cnt, sizeof(*sizes));
